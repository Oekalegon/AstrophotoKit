...
/// Processor for applying binary dilation to frames
public struct DilationProcessor: Processor {

    public var id: String { "dilation" }

    public init() {}

    /// Execute the dilation processor
    /// - Parameters:
    ///   - inputs: Dictionary containing "input_frame" -> ProcessData (Frame)
    ///   - outputs: Dictionary containing "dilated_frame" -> ProcessData (Frame, to be instantiated)
    ///   - parameters: Dictionary containing:
    ///     - "kernel_size" -> Parameter (Int, default: 3) - size of the dilation kernel
    ...
    public func execute(
        ...
        Logger.processor.debug("Applying binary dilation with kernel size: \(kernelSize)")

        // Load shader library and function
        let library = try ProcessorHelpers.loadShaderLibrary(device: device)
        guard let dilationFunction = library.makeFunction(name: "binary_dilation") else {
            throw ProcessorExecutionError.couldNotCreateResource("Could not load binary_dilation shader function")
        }

        // Create compute pipeline state
        let computePipelineState = try ProcessorHelpers.createComputePipelineState(
            function: dilationFunction,
            device: device
        )
        ...
        try ProcessorHelpers.updateOutputFrame(
            outputs: &outputs,
            outputName: "dilated_frame",
            texture: outputTexture
        )

        Logger.processor.info("Dilation completed successfully (kernel size: \(kernelSize))")
    }
}
