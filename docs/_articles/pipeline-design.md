...
Before we dive in, let's establish some key terminology. We'll use the word **frame** to refer to any single image, whether it's captured directly by your camera, generated by a processing step, or even a calibration frame like dark frames or bias frames. An **astrophoto**, on the other hand, is the final result of carefully orchestrated operations on one or more frames. Think of frames as the raw materials and an astrophoto as the finished artwork.
...
Steps can produce multiple outputs, and different steps can consume different subsets of those outputs. For instance, step A might produce outputs X and Y. Step B could then use only output X to produce Z, while step C uses only output Y to produce W. Since steps B and C have no dependencies on each other (they operate on independent inputs from step A), they can execute in parallel, improving overall pipeline performance. Step D could then use both Z and W (from steps B and C respectively) to produce the final output V.

The pipeline executor uses a data-driven execution model rather than building a dependency graph upfront. The executor maintains a list of available data (pipeline inputs and step outputs) and tracks pending step instances. When new data becomes available, the executor checks all pending step instances to determine if their required inputs are now satisfied. This approach naturally supports parallel execution of independent steps and multiple instances of the same step processing different inputs.

### Step Instance Identification

Each processing step execution receives a unique identifier that distinguishes it from other instances, even when the same step configuration is applied to different inputs. For example, when processing three images (X, Y, Z) through a blur step (A), three distinct step instances are created with unique IDs (e.g., "blur_X", "blur_Y", "blur_Z"). This allows the same step configuration to process multiple inputs in parallel while maintaining clear separation between instances.

This unique identification is essential for scenarios where multiple frames go through the same processing steps before being combined. For instance, in a stacking pipeline, each light frame may go through identical calibration and preprocessing steps (blur, background subtraction, etc.) in parallel, and then all processed frames are combined in a stacking step. The executor can start processing all frames through the initial steps simultaneously, but must wait for all frames to complete before starting the stacking step that requires all inputs.
...
Each processor instance is wrapped in a `ProcessorInstance` object that tracks the unique identifier, current status (pending, running, completed, or failed), and progress (0.0 to 1.0) for that specific execution. When multiple instances of the same step process different inputs (e.g., processing three images through a blur step), each instance maintains its own status and progress tracking. This allows GUI applications to display detailed progress information for each individual processor instance, showing which specific inputs are being processed and their current status. The processor instance wrapper also ensures that outputs are uniquely identified per instance, so that processing step A on image X produces output U, while processing step A on image Y produces output V, and these are treated as distinct outputs.

Each `ProcessorInstance` maintains a list of log entries that record events and messages during execution. Log entries include a timestamp, severity level (debug, info, warning, error, or fatal), message text, and optional contextual metadata. This per-instance logging allows for detailed tracking of what happened during each processor execution, enabling debugging, auditing, and user feedback. Log entries can be filtered by severity level, allowing applications to focus on warnings and errors when needed, or to provide comprehensive debugging information during development. The logging system is thread-safe and can be accessed at any time to review the execution history of a processor instance.

### Error Handling and Pipeline Continuation

The pipeline system distinguishes between fatal and non-fatal errors during execution. Fatal errors (such as processor not found, Metal unavailable, or resource creation failures) cause the entire pipeline to stop immediately, as these indicate conditions where continued execution would be unsafe or produce invalid results. Non-fatal errors (such as execution failures for specific inputs or missing inputs for a particular instance) allow the pipeline to continue processing other independent instances.

When a processor instance encounters a non-fatal error, it is marked as failed and its outputs are not produced. Any downstream processor instances that depend on the failed instance's outputs will remain pending and will not execute, effectively halting only the processing paths that depend on the failed instance. This design allows the pipeline to gracefully handle failures in individual processing paths (e.g., a single frame failing to process) while continuing to process other independent paths (e.g., other frames in a collection). For example, if processing step A on image X fails with a non-fatal error, only the processing paths that depend on the output from that instance are halted, while processing step A on images Y and Z can continue independently. 
